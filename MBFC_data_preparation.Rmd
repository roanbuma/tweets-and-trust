...
Removed 366 unvalid cases (NA for category)

# Adding missing sources manually
...
## orientation (new variable)

Detect occurance of "Right", "Left", and "Least" (biased) in bias ratings; assign to new variable "orientation"
...
df$mbfc_credibility_rating = df$mbfc_credibility_rating %>%
  ...
  str_replace("high$", "high credibility")
...
There are some duplicate sources with different categories. 

First, we filter out duplicates if they are an extension of the main website.
...
Second, we filter out remaining duplicates by choosing the most recent instance.
...
For any remaining duplicates, we checked whether the category is the same. If so, we removed the duplicate. If the category of the duplicate is not the same, we removed both records (2 cases in the data)
